...
$potenciasUnicas = [];
$stmt = $pdo->query("SELECT DISTINCT potencia_maxima FROM fichas_tecnicas WHERE potencia_maxima IS NOT NULL AND potencia_maxima != '' ORDER BY potencia_maxima");
...
if (!empty($_GET['busca'])) {
    ...
    $whereConditions[] = "(m.nome LIKE ? OR f.modelo LIKE ? OR f.versao LIKE ?)";
    $params[] = $busca;
    $params[] = $busca;
    $params[] = $busca;
}
...
if (!empty($_GET['marca'])) {
    $whereConditions[] = "f.marca_id = ?";
    $params[] = sanitizeInput($_GET['marca']);
}

if (!empty($_GET['combustivel'])) {
    $whereConditions[] = "f.tipo_combustivel = ?";
...
if (!empty($_GET['potencia'])) {
    ...
    if ($potencia === 'ate_100') {
        $whereConditions[] = "CAST(REGEXP_REPLACE(f.potencia_maxima, '[^0-9]', '') AS UNSIGNED) <= 100";
    } elseif ($potencia === '100_200') {
        $whereConditions[] = "CAST(REGEXP_REPLACE(f.potencia_maxima, '[^0-9]', '') AS UNSIGNED) BETWEEN 100 AND 200";
    } elseif ($potencia === 'acima_200') {
        $whereConditions[] = "CAST(REGEXP_REPLACE(f.potencia_maxima, '[^0-9]', '') AS UNSIGNED) > 200";
    }
}

// Buscar fichas
$fichas = [];
$sql = "SELECT f.*, m.nome as marca_nome, c.nome as carroceria_nome 
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
            ...
            <form method="GET" class="space-y-4">
                <div class="flex flex-col md:flex-row gap-4 mb-6">
                    <div class="flex-1">
                        <input type="text" name="busca" id="searchInput" placeholder="Pesquisar por modelo, marca ou versão..." 
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="fichas.php" class="filter-btn px-4 py-2 rounded-full bg-gray-200 text-gray-700 text-sm hover:bg-gray-300 transition-colors <?php echo empty($_GET['marca']) ? 'active bg-senai-blue text-white' : ''; ?>">
                        Todas
                    </a>                    
                    <?php foreach ($marcas as $marca): ?>
                        <a href="fichas.php?marca=<?php echo $marca['id']; ?>" class="filter-btn px-4 py-2 rounded-full bg-gray-200 text-gray-700 text-sm hover:bg-gray-300 transition-colors <?php echo (isset($_GET['marca']) && $_GET['marca'] == $marca['id']) ? 'active bg-senai-blue text-white' : ''; ?>">
                            <?php echo htmlspecialchars($marca['nome']); ?>
                        </a>
                    <?php endforeach; ?>
                </div>
        ...
        <div class="mb-6">
            <p class="text-gray-600">
                ...
                <?php if (!empty($_GET['busca']) || !empty($_GET['carroceria']) || !empty($_GET['marca']) || !empty($_GET['combustivel']) || !empty($_GET['ano_inicio']) || !empty($_GET['potencia'])): ?>
                        ...
                        <?php if ($ficha['imagem_path']): ?>
                            <div class="h-48 bg-gray-200 overflow-hidden">
                                <img src="<?php echo htmlspecialchars($ficha['imagem_path']); ?>" 
                                     alt="<?php echo htmlspecialchars($ficha['marca_nome'] . ' ' . $ficha['modelo']); ?>" 
                            ...
                            <div class="space-y-2 mb-6">
                                <?php if ($ficha['tipo_motor']): ?>
                                    <div class="flex justify-between text-sm">
                                        ...
                                        <span class="font-medium"><?php echo htmlspecialchars($ficha['tipo_motor']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if ($ficha['potencia_maxima']): ?>
                                    <div class="flex justify-between text-sm">
                                        ...
                                        <span class="font-medium"><?php echo htmlspecialchars($ficha['potencia_maxima']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if ($ficha['tipo_cambio']): ?>
                                    <div class="flex justify-between text-sm">
                                        ...
                                        <span class="font-medium"><?php echo htmlspecialchars($ficha['tipo_cambio']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if ($ficha['tipo_combustivel']): ?>
                                    <div class="flex justify-between text-sm">
                                        ...
                                        <span class="font-medium"><?php echo htmlspecialchars($ficha['tipo_combustivel']); ?></span>
                                    </div>
                                <?php endif; ?>
                            </div>
