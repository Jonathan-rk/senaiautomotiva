...
function uploadImage($file, $uploadDir = 'uploads/marcas/') {
...
// Processar adição/edição de marca
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $nome = sanitizeInput($_POST['nome']);
    $marcaId = isset($_POST['id']) ? sanitizeInput($_POST['id']) : null;
    $imagemPath = null;
        ...
        try {
            ...
            if ($marcaId) {
                ...
                if ($imagemPath) {
                    ...
                    $stmt = $pdo->prepare("SELECT imagem_path FROM marcas WHERE id = ?");
                    ...
                    $stmt = $pdo->prepare("UPDATE marcas SET nome = ?, imagem_path = ? WHERE id = ?");
                    $stmt->execute([$nome, $imagemPath, $marcaId]);
                } else {
                    $stmt = $pdo->prepare("UPDATE marcas SET nome = ? WHERE id = ?");
                    $stmt->execute([$nome, $marcaId]);
                }
                $success = 'Marca atualizada com sucesso!';
            } else {
                // Adicionar nova marca
                $stmt = $pdo->prepare("INSERT INTO marcas (nome, imagem_path) VALUES (?, ?)");
                $stmt->execute([$nome, $imagemPath]);
                $success = 'Marca adicionada com sucesso!';
            }
        } catch (Exception $e) {
            $error = $e->getMessage();
            ...
            if ($e->getCode() == '23000') {
                ...
                $error = 'Erro ao salvar marca: ' . $e->getMessage();
            }
        }
    }
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    $marcaId = sanitizeInput($_GET['delete']);
    try {
        ...
        $error = 'Erro ao excluir marca: ' . $e->getMessage();
    }
}

// Carregar marca para edição
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    $marcaId = sanitizeInput($_GET['edit']);
    $stmt = $pdo->prepare("SELECT * FROM marcas WHERE id = ?");
    $stmt->execute([$marcaId]);
    $editMarca = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$editMarca) {
        $error = 'Marca não encontrada para edição.';
    }
}
...
<head>
    ...
    <title>Gerenciar Marcas - SENAI Automotivo</title>
        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex justify-between items-center py-4">
                ...
                <div class="hidden md:flex items-center space-x-6">
                    ...
                    <a href="marcas.php" class="text-blue-600 font-semibold">
            ...
            <div id="mobileMenu" class="hidden md:hidden pb-4">
                ...
                <a href="marcas.php" class="block py-2 text-blue-600 font-semibold">
                    <i class="fas fa-car mr-2"></i>Marcas
                </a>
                <a href="carrocerias.php" class="text-gray-700 hover:text-blue-600 transition-colors">
                    <i class="fas fa-shapes mr-2"></i>Carrocerias
        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                ...
                <h1 class="text-3xl font-bold mb-2 text-center flex justify-center items-center">
                    <i class="fas fa-car mr-3"></i>Gerenciar Marcas
                </h1>
                    <p class="text-lg opacity-90">
                        Adicione, edite ou exclua marcas de veículos
                    </p>
                </div>
        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
                <h2 class="text-2xl font-bold senai-blue mb-6">
                    <i class="fas fa-<?php echo $editMarca ? 'edit' : 'plus'; ?>-circle mr-2"></i>
                    <?php echo $editMarca ? 'Editar Marca' : 'Adicionar Nova Marca'; ?>
                </h2>
                <form method="POST" action="marcas.php" enctype="multipart/form-data" class="space-y-6">
                    <?php if ($editMarca): ?>
                        <input type="hidden" name="id" value="<?php echo htmlspecialchars($editMarca['id']); ?>">
                    <?php endif; ?>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="nome" class="block text-sm font-medium text-gray-700 mb-1">Nome da Marca *</label>
                            <input type="text" id="nome" name="nome" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-senai-blue focus:border-senai-blue sm:text-sm" 
                                   value="<?php echo htmlspecialchars($editMarca['nome'] ?? ''); ?>" required>
                        </div>
                        
                        <div>
                            <label for="imagem" class="block text-sm font-medium text-gray-700 mb-1">Imagem da Marca</label>
                            ...
                            <?php if ($editMarca && $editMarca['imagem_path']): ?>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-600">Imagem atual:</p>
                                    <img src="<?php echo htmlspecialchars($editMarca['imagem_path']); ?>" alt="<?php echo htmlspecialchars($editMarca['nome']); ?>" class="w-20 h-20 object-contain border rounded">
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-4">
                        <button type="submit" class="px-6 py-3 bg-senai-blue text-white rounded-lg hover:bg-blue-700 transition-colors font-semibold">
                            <i class="fas fa-save mr-2"></i><?php echo $editMarca ? 'Atualizar' : 'Adicionar'; ?> Marca
                        </button>
                        <?php if ($editMarca): ?>
                            <a href="marcas.php" class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors font-semibold">
                                Cancelar
                            </a>
                        <?php endif; ?>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                <h2 class="text-2xl font-bold senai-blue mb-6">
                    <i class="fas fa-list mr-2"></i>Marcas Cadastradas
                </h2>
                <?php if (empty($marcas)): ?>
                    <div class="text-center py-12">
                        <i class="fas fa-car text-6xl text-gray-300 mb-4"></i>
                        <h3 class="text-xl font-semibold text-gray-600 mb-2">Nenhuma marca cadastrada</h3>
                        <p class="text-gray-500">Adicione sua primeira marca acima.</p>
                    </div>
                <?php else: ?>
                        ...
                        <?php foreach ($marcas as $marca): ?>
                            <div class="bg-gray-50 rounded-lg p-4 hover:shadow-md transition-shadow">
                                <div class="text-center">
                                    <?php if ($marca['imagem_path']): ?>
                                        <img src="<?php echo htmlspecialchars($marca['imagem_path']); ?>" 
                                    ...
                                    <h3 class="font-semibold text-gray-900 mb-3"><?php echo htmlspecialchars($marca['nome']); ?></h3>
                                    
                                    <div class="flex justify-center space-x-2">
                                        <a href="marcas.php?edit=<?php echo $marca['id']; ?>" class="px-3 py-1 bg-green-500 text-white text-sm rounded hover:bg-green-600 transition-colors">
                                            <i class="fas fa-edit"></i> Editar
                                        </a>
                                        <a href="marcas.php?delete=<?php echo $marca['id']; ?>" onclick="return confirm('Tem certeza que deseja excluir esta marca? Isso também removerá fichas técnicas associadas a ela.');" class="px-3 py-1 bg-red-500 text-white text-sm rounded hover:bg-red-600 transition-colors">
