...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    try {
        // Buscar imagem da montadora para deletar
        $stmt = $pdo->prepare("SELECT imagem_path FROM montadoras WHERE id = ?");
        $stmt->execute([$montadorasId]);
        $imagePath = $stmt->fetchColumn();

        // Buscar imagens das fichas técnicas associadas a essa montadora
        $stmtFichas = $pdo->prepare("SELECT imagem_path FROM fichas_tecnicas WHERE montadoras_id = ?");
        $stmtFichas->execute([$montadorasId]);
        $fichasImagens = $stmtFichas->fetchAll(PDO::FETCH_COLUMN);

        // Excluir imagem da montadora (se for local)
        if ($imagePath && file_exists($imagePath) && strpos($imagePath, 'uploads/') === 0) {
            unlink($imagePath);
        }

        // Excluir imagens das fichas técnicas associadas (se for local)
        foreach ($fichasImagens as $fichaImg) {
            if ($fichaImg && file_exists($fichaImg) && strpos($fichaImg, 'uploads/') === 0) {
                unlink($fichaImg);
            }
        }

        // Excluir fichas técnicas associadas (ON DELETE CASCADE já faz isso, mas garantimos as imagens)
        // Excluir montadora
...
<body class="bg-gray-50">
      ...
      <div class="container">
        <div class="navbar-header">
          ...
          <div class="menu-desktop">
            ...
            <a href="montadoras.php" class="active"><i class="fas fa-car icon"></i>Montadoras</a>
    ...
    <section class="py-8">
        <div class="max-w-7xl mx-auto px-4">
            ...
            <!-- Formulário Adicionar/Editar Montadoras -->
                                ...
                                <div class="text-center">
                                    ...
                                    <div class="flex justify-center space-x-2">
                                        ...
                                        <button type="button"
                                            ...
                                            onclick="showDeleteModal(<?php echo $montadoras['id']; ?>, '<?php echo htmlspecialchars(addslashes($montadoras['nome'])); ?>')">
...
<footer class="footer">
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
        <form method="GET" action="montadoras.php" class="bg-white rounded-lg shadow-lg p-8 max-w-sm w-full">
            <h3 class="text-lg font-bold text-gray-800 mb-4">Confirmar exclusão</h3>
            <p class="mb-6 text-gray-700">Tem certeza que deseja excluir a montadora <span id="modalMontadoraNome" class="font-semibold"></span>?<br>
            Isso também removerá fichas técnicas associadas a ela.</p>
            <input type="hidden" name="delete" id="modalMontadoraId" value="">
    ...
    function showDeleteModal(id, nome) {
        document.getElementById('modalMontadoraNome').textContent = nome;
        document.getElementById('modalMontadoraId').value = id;
